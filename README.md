...
- Use the admin/staff creation scripts to create management accounts
- Or use test accounts created by seed scripts (if included)
...
3. **Configure environment variables**:
   - `DATABASE_URL`: MySQL connection string (or leave unset for SQLite)
   - `SECRET_KEY`: Flask secret key for sessions
4. **Configure WSGI file** to point to `hotelweb.app:create_app()`
5. **Set Static Files mapping**: 
   - URL `/static/` → Directory `/home/yourusername/hotel/static/`
6. **Run database migrations** (if needed) or use `db.create_all()` in app context
